...
void ADVSCamera::Tick(float DeltaTime)
{
  ...
  //std::string path = "/home/javi/Pictures/frames/frame_"+std::to_string(idx)+".png";
  //FString string_path; string_path = path.c_str();
  //this->SaveCaptureToDisk(string_path);
  //idx++;
  ...
  if (events.size() > 0)
  {
    //std::cout<<"image size() "<<this->prev_image.Num()<<" image size() "<<this->last_image.Num()<<" events size() "<<events.size()<<std::endl;
    //std::cout<<"Current Time: "<<this->current_time<<std::endl;
    //std::cout<<"Time: "<<dvs::secToNanosec(this->GetEpisode().GetElapsedGameTime())<<std::endl;
    //std::cout<<"[0]:"<<events[0].x<<","<<events[0].y<<","<<events[0].t<<","<<events[0].pol<<std::endl;
    //std::cout<<"["<<events.size()-1<<"]:"<<events[events.size()-1].x<<","<<events[events.size()-1].y<<","<<events[events.size()-1].t<<","<<events[events.size()-1].pol<<std::endl;
...
ADVSCamera::DVSEventArray ADVSCamera::simulation (float DeltaTime)
{
  ...
  //std::cout<<"["<<DeltaTime<<"] start of simulation loop[current_time]: "<<this->current_time<<std::endl;
  ...
  //std::cout<<"\nend of simulation loop[current_time]: "<<this->current_time<<std::endl;

  /** TO_DO: Move last image for next iteration (move semantic) **/
  this->prev_image = this->last_image;
  ...
  //std::sort(events.begin(), events.end(), [](const ::carla::sensor::data::DVSEvent& it1, const ::carla::sensor::data::DVSEvent& it2){return it1.x > it2.x;});
